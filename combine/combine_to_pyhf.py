...
except ImportError: # numpy v1
    from numpy.core.fromnumeric import size

try:
    import DatacardParser as DP
    from Datacard import Datacard
except ImportError as e:
    ## seems like we are not working in a local install, lets see if combine is there:
    try:
...
class CombineToPyhf:
    ...
    def pprint ( self, *args ):
        print ( f"[combine_to_pyhf] {' '.join(args)}" )

    def getShapeFile(self, shapeMap: dict, channel, sample) -> string:
        """
    ...
    def getHistPath( self, shapeMap: dict, channel, sample) -> string:
        ##get path to sample histogram
        path = ""
    ...
    def getUncertPath( self, shapeMap: dict, channel, sample, name) -> string:
        ##get path to shape uncertainty if it exists
        path = ""
        if not channel in shapeMap.keys():
            if "*" in shapeMap.keys():
                if not sample in shapeMap["*"]:
    ...
    def getHist( self, shapeMap: dict, channel, sample):
        ##get actual sample histogram
    ...
    def addChannels( self, spec: dict, data_card, channels, observations):
        """
    ...
    def addSamples( self, spec: dict, data_card, channels, samples, exp_values):
        """
    ...
    def addMeasurements( self, spec: dict, data_card, channels, samples, sig):
        """
    ...
    def addNormFactor( self, spec: dict, data_card, channels, samples, sig):
        """
    ...
    def addMods( self, spec: dict, data_card, channels, samples, exp_values, mods):
        """
    ...
    def convert( self, input_datacard, outfile, options=None):
        """ convert the input combine datacard to pyhf json file """
        ...
        self.addChannels(spec, data_card, channels, observations)
        self.addSamples(spec, data_card, channels, samples, exp_values)
        self.addMeasurements(spec, data_card, channels, samples, sig)
        self.addNormFactor(spec, data_card, channels, samples, sig)
        self.addMods(spec, data_card, channels, samples, exp_values, mods)

        with open(outfile, "w") as file:
            file.write(json.dumps(spec, indent=2))
        self.pprint ( f"{outfile} created" )      

def main():
    from optparse import OptionParser
    # Add command line args
    parser = OptionParser(usage='usage: %prog [options] combine_datacard')
    ...
    if len(args)==0:   # if filename is not given
        parser.error('combine_datacard name not given')

    converter = CombineToPyhf()
    converter.convert(
