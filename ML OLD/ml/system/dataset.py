...
class Data(Dataset):
	...
	def __init__(self, dataset, inputDimension, device, rescaleInputs = False, rescaleLabels = False):
		...
		self.inputs = torch.tensor(dataset).narrow(1, 0, inputDimension).to(device)
		self.labels = torch.tensor(dataset).narrow(1, inputDimension, 1).to(device)

		self.mean = [torch.mean(self.inputs), torch.mean(self.labels)]
		self.std = [torch.std(self.inputs), torch.std(self.labels)]
...
def _getExpresData(expres, txName):
...
def generateDataset(expres, topo, massRange, sampleSize, dataType, device, shuffleData=True):
	...
	for tx in analysis.txnameList: 
		if tx == topo or tx.txName == topo:
			...
			expresData 			= _getExpresData(expres, topo)
