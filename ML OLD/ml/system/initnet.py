...
def saveModel(model, expres, txName, netType):
	...
	fileName = txName + '_' + netType + '.pth'
	torch.save(model, savePath + fileName)
	print("Model saved at %s" %(savePath + fileName))
...
def createNet(hyperParameter, inputDimension, netType, rescaleParameter):
	...
	netshape, nodesTotal = getNodesPerLayer(shape, nodes, layer, inputDimension)

	if netType == 'regression':
		model = Net_reg(netshape, activationFunction, rescaleParameter)
	elif netType == 'classification':
		model = Net_cla(netshape, activationFunction)
