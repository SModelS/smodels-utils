...
#Given a SLHA file for a proto-model converts it to the SLHA2 format used by MadGraph
#All the code below is from MadGraph5 (models/check_param_card.py), except for the
#following changes:

# 1) Added help information
# 2) Include SM decay blocks
# 3) Removed unnecessary classes and functions
# 4) Added missing decay blocks (with zero width)
...
def convert_to_mg5card(path, outputpath=None, writting=True):
    ...
    if ae * ye:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter ae [1, 1] times ye [1,1] should be 0''')
    ...
    if ae * ye:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter ae [2, 2] times ye [2,2] should be 0''')
    ...
    if au * yu:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter au [1, 1] times yu [1,1] should be 0''')
    ...
    if au * yu:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter au [2, 2] times yu [2,2] should be 0''')
    ...
    if ad * yd:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter ad [1, 1] times yd [1,1] should be 0''')
    ...
    if ad * yd:
        raise InvalidParamCard('''This card is not suitable to be converted to MSSM UFO model Parameter ad [2, 2] times yd [2,2] should be 0''')
    ...
    #Get pre-existing decay blocks:
    decayBlocks = [b.lhacode[0] for b in card['decay']]
    #SM decay blocks
    #light quark widths:
    for pdg in range(1,6):
        if pdg in decayBlocks: continue #file already contains the decay
        dblock = Parameter(block='decay', lhacode=(pdg,), value=0.0, comment='SM decay')
        card['decay'].append(dblock)

    #top quark width:
    pdg = 6
    if not pdg in decayBlocks:
        dblock = Parameter(block='decay', lhacode=(pdg,), value=1.466877e+00, comment='SM decay')
        card['decay'].append(dblock)

    #lepton widths:
    for pdg in range(11,17):
        if pdg in decayBlocks: continue #file already contains the decay
        dblock = Parameter(block='decay', lhacode=(pdg,), value=0.0, comment='SM decay')
        card['decay'].append(dblock)

    #gauge boson widths
    for pdg in range(21,23):
        if pdg in decayBlocks: continue #file already contains the decay
        dblock = Parameter(block='decay', lhacode=(pdg,), value=0.0, comment='SM decay')
        card['decay'].append(dblock)
    pdg = 23
    if not pdg in decayBlocks:
        dblock = Parameter(block='decay', lhacode=(pdg,), value=2.411596e+00, comment='SM decay')
        card['decay'].append(dblock)

    #Update the list of existing decay blocks:
    decayBlocks = [b.lhacode[0] for b in card['decay']]

    #Make sure all particles in the mass block have a decay block:
    for mb in card['mass']:
        pdg = mb.lhacode[0]
        if pdg in decayBlocks: continue
        dblock = Parameter(block='decay', lhacode=(pdg,), value=0.0, comment=mb.comment)
        card['decay'].append(dblock)

    #Make sure all particles in the decay block have a mass entry:
    massEntries = [m.lhacode[0] for m in card['mass']]
    massDict = {23: 91.2, 24: 80.4, 25: 125.1, 6: 173.1, 5: 4.7}
    for pdg in decayBlocks:
        if pdg in massEntries: continue
        if pdg in massDict:
            mass = massDict[pdg]
        else:
            mass = 0.0
        mblock = Parameter(block='mass', lhacode=(pdg,), value=mass, comment='missing mass')
        card['mass'].append(mblock)
